...
async function drawFollowerImage(follower, fileName) {
  if (!follower) {
    console.warn('Follower is null or undefined. Skipping image creation.');
    return;
  }

  try {
    if (!follower.avatar_url || typeof follower.avatar_url !== 'string') {
      throw new Error('Avatar URL is missing or invalid.');
    }
    ...
    console.error('Erro ao criar a imagem do seguidor:', error);
  }
}
